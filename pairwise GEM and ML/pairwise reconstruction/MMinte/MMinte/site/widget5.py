...
def calculateGR(diet, outputGRs, comFolder):
    ...
    cherrypy.log(
        'We will now calculate the growth rates of the two species in a community model in the presence and absence of the other species')
    ...
    for item in range(len(allModels)):
        ...
        cherrypy.log(
            'We successfully loaded the file %s into three different Model objects with ids, %s, %s, and %s. They should all have the same id.' % (
            allModels[item], modelFull.id, modelMinusA.id, modelMinusB.id))
        ...
        cherrypy.log(
            'The objective function of the full model, %s, contains the objective functions of the two models that make it up. They are %s.' % (
            allModels[item], idObjKeys))
        ...
        cherrypy.log(
            'We finished changing the lower bounds for the fluxes of the exchange reactions in the models to better fit the availability of metabolites for the microbial communities we are simulating the growth of. ')

        # Run FBA on Full model
        modelFull.optimize()

        cherrypy.log(
            'We finished running the FBA on the full model. The status of the solution is %s and the value of the solution found is %f' % (
            modelFull.solution.status, modelFull.solution.f))
        ...
        cherrypy.log(
            'We finished removing all reactions from the model. The model now has %s reactions and the reaction on the objective function in %s' % (
            len(modelMinusA.reactions), modelMinusA.objective))

        modelMinusA.optimize()

        cherrypy.log(
            'We finished running the FBA on the model without reactions of species A. The status of the solution is %s and the value of the solution found is %f' % (
            modelMinusA.solution.status, modelMinusA.solution.f))
        ...
        cherrypy.log(
            'We finished removing all reactions from the model. The model now has %s reactions and the reaction on the objective function in %s' % (
            len(modelMinusB.reactions), modelMinusB.objective))

        modelMinusB.optimize()

        cherrypy.log(
            'We finished running the FBA on the model without reactions of species A. The status of the solution is %s and the value of the solution found is %f' % (
            modelMinusB.solution.status, modelMinusB.solution.f))
        ...
        cherrypy.log(
            'We matched the reactions in the objective function to the model they came from. %s was originally from modelA and %s was originally from mobelB' % (
            ObjA, ObjB))
        ...
        cherrypy.log(
            "We are going to create a table with the information about the growth of A and B alone and in the presence of B and A respectively.")
        ...
        if grAMinusA != 'Solo' or grBMinusB != 'Solo':
            cherrypy.log(
                'There is a problem with the attribution of growth rate values to their respective species in model %s .' % modelFull.id)
        ...
        print(modelID, '\t', organisms[0], '\t', organisms[
            1], '\t', grAfull, '\t', grBfull, '\t', grAMinusB, '\t', grBMinusA, file=growthRatesFile)

    cherrypy.log(
        'We finished calculating the growth rates of the species in isolation and when in the presence of another species and dumped the information to the file: %s' % growthRatesFile)
